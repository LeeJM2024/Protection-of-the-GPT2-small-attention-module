...
typedef struct {
    ...
    float* ln1w;       // block中的第一层归一化权重，形状为(L, C)，
    // 因为每一个block的权重都不一样，所以需要有L列、C行，L代表block数量，以下所有带L的同理，都是block中的层
...
typedef struct {
    ...
    int num_layers;     // 注意力层数，代表一共有多少个block
    int num_heads;      // 注意力头数
...
typedef struct {
    ...
    int trusted_layer;                          // 存入TA的层，1表示encoder层
} GPT2;
