...
/**
 * @brief 发送“带输入张量”的保护块前向命令到TA，代表这一步需要从CA喂入一个新的张量
 * @param cmd TA处将要执行的操作
 * @param inp 输入张量缓冲区
 * @param inp_bytes 输入张量字节数
 * @param B 批次大小
 * @param T 序列长度
 * @param C 通道数（隐藏层维度）
 * @param extra 额外参数（按具体命令复用）
 */
static void block_forward_input_CA(uint32_t cmd, float *inp, size_t inp_bytes, int B, int T, int C, int extra) {
    TEEC_Operation op = {0};
    uint32_t err_origin;
    TEEC_Result res;
    ...
    op.paramTypes = TEEC_PARAM_TYPES(TEEC_MEMREF_TEMP_INPUT, TEEC_VALUE_INPUT, TEEC_VALUE_INPUT, TEEC_NONE);
    op.params[0].tmpref.buffer = inp;
    op.params[0].tmpref.size = (uint32_t)inp_bytes;
    op.params[1].value.a = (uint32_t)B;
    op.params[1].value.b = (uint32_t)T;
    op.params[2].value.a = (uint32_t)C;
    op.params[2].value.b = (uint32_t)extra;

    res = TEEC_InvokeCommand(&sess, cmd, &op, &err_origin);
    if (res != TEEC_SUCCESS)
        errx(1, "BLK forward(input) cmd=0x%x failed: 0x%x origin 0x%x", cmd, res, err_origin);
}

/**
 * @brief 发送“纯VALUE参数”的保护块前向命令到TA，代表这一步只需要TA用自己内部缓存接着算，因为每一步的TA算完后都存在TA缓冲区
 * @param cmd TA处将要执行的操作
 * @param B 批次大小
 * @param T 序列长度
 * @param C 通道数（隐藏层维度）
 * @param extra 额外参数（按具体命令复用）
 */
static void block_forward_value_CA(uint32_t cmd, int B, int T, int C, int extra) {
    TEEC_Operation op = {0};
    uint32_t err_origin;
    TEEC_Result res;

    op.paramTypes = TEEC_PARAM_TYPES(TEEC_VALUE_INPUT, TEEC_VALUE_INPUT, TEEC_NONE, TEEC_NONE);
    op.params[0].value.a = (uint32_t)B;
    op.params[0].value.b = (uint32_t)T;
    op.params[1].value.a = (uint32_t)C;
    op.params[1].value.b = (uint32_t)extra;

    res = TEEC_InvokeCommand(&sess, cmd, &op, &err_origin);
    if (res != TEEC_SUCCESS)
        errx(1, "BLK forward(value) cmd=0x%x failed: 0x%x origin 0x%x", cmd, res, err_origin);
}

/**
 * @brief 从TA取回最后residual3层的输出张量
 * @param out 输出张量缓冲区
 * @param B 批次大小
 * @param T 序列长度
 * @param C 通道数（隐藏层维度）
 */
static void block_output_CA(float *out, int B, int T, int C) {
    TEEC_Operation op = {0};
    uint32_t err_origin;
    TEEC_Result res;
    ...
    op.paramTypes = TEEC_PARAM_TYPES(TEEC_MEMREF_TEMP_OUTPUT, TEEC_NONE, TEEC_NONE, TEEC_NONE);
    op.params[0].tmpref.buffer = out;
    op.params[0].tmpref.size = (uint32_t)((size_t)B * T * C * sizeof(float));

    res = TEEC_InvokeCommand(&sess, TA_GPT_CMD_BLOCK_OUTPUT, &op, &err_origin);
    if (res != TEEC_SUCCESS)
        errx(1, "BLK output failed: 0x%x origin 0x%x", res, err_origin);
}

/**
 * @brief 保护块中的LN1前向（输入residual）
 */
void ln1_forward_CA(float *residual, int B, int T, int C) { block_forward_input_CA(TA_GPT_CMD_LN1_FORWARD, residual, (size_t)B * T * C * sizeof(float), B, T, C, 0); }
/**
 * @brief 保护块中的QKV线性层前向
 */
void qkv_forward_CA(int B, int T, int C) { block_forward_value_CA(TA_GPT_CMD_QKV_FORWARD, B, T, C, 0); }
/**
 * @brief 保护块中的自注意力前向
 */
void attention_forward_CA(int B, int T, int C, int NH) { block_forward_value_CA(TA_GPT_CMD_ATTENTION_FORWARD, B, T, C, NH); }
/**
 * @brief 保护块中的注意力投影前向
 */
void attproj_forward_CA(int B, int T, int C) { block_forward_value_CA(TA_GPT_CMD_ATTPROJ_FORWARD, B, T, C, 0); }
/**
 * @brief 保护块中的第一次残差相加前向
 */
void residual2_forward_CA(float *residual, int B, int T, int C) { block_forward_input_CA(TA_GPT_CMD_RESIDUAL2_FORWARD, residual, (size_t)B * T * C * sizeof(float), B, T, C, 0); }
/**
 * @brief 保护块中的LN2前向
 */
void ln2_forward_CA(int B, int T, int C) { block_forward_value_CA(TA_GPT_CMD_LN2_FORWARD, B, T, C, 0); }
/**
 * @brief 保护块中的FFN第一层全连接前向
 */
void fc_forward_CA(int B, int T, int C) { block_forward_value_CA(TA_GPT_CMD_FC_FORWARD, B, T, C, 0); }
/**
 * @brief 保护块中的GELU激活前向
 */
void gelu_forward_CA(int B, int T, int C) { block_forward_value_CA(TA_GPT_CMD_GELU_FORWARD, B, T, C, 0); }
/**
 * @brief 保护块中的FFN投影层前向
 */
void fcproj_forward_CA(int B, int T, int C) { block_forward_value_CA(TA_GPT_CMD_FCPROJ_FORWARD, B, T, C, 0); }
/**
 * @brief 保护块中的第二次残差相加前向
 */
void residual3_forward_CA(int B, int T, int C) { block_forward_value_CA(TA_GPT_CMD_RESIDUAL3_FORWARD, B, T, C, 0); }
...
void load_lnfwb_CA(GPT2 *model, FILE *model_file) { 
    ...
    printf("[CA] load_lnfwb: lnfw=%d floats, lnfb=%d floats\n", lnfw_elements, lnfb_elements);   
...
/**
 * @brief 加载注意力blcock中指定层的权重和偏置到TA（CA实现）
 * @param buf 指向需要传输的受保护张量数据缓冲区的指针（作为临时输入共享内存传递）
 * @param bytes 需要传输的数据块总大小
 * @param C 模型的特征维度或通道数
 * @param layer_id 当前数据块所属的受保护层索引
 * @param cmd TA处将要执行的操作
 * @param name 当前操作的名称标识
 */
static void load_block_tensor_CA(const float *buf, size_t bytes, int C, int layer_id, uint32_t cmd, const char *name) {
    TEEC_Operation op = {0};
    uint32_t err_origin;
    TEEC_Result res;
    ...
    op.params[0].tmpref.size = (uint32_t)bytes;
    op.params[1].value.a = (uint32_t)C;
    op.params[1].value.b = (uint32_t)layer_id;

    res = TEEC_InvokeCommand(&sess, cmd, &op, &err_origin);
    if (res != TEEC_SUCCESS)
        errx(1, "%s failed: 0x%x origin 0x%x", name, res, err_origin);
}

/**
 * @brief 加载注意力block中的第1个LayerNorm的权重、偏置到TA (形状: C)
 * @param buf 数据缓冲区指针
 * @param C 特征维度
 * @param layer_id 目标层索引
 */
void load_ln1w_CA(const float *buf, int C, int layer_id) { load_block_tensor_CA(buf, (size_t)C * sizeof(float), C, layer_id, TA_GPT_CMD_LOAD_LN1W, "LOAD_LN1W"); }
void load_ln1b_CA(const float *buf, int C, int layer_id) { load_block_tensor_CA(buf, (size_t)C * sizeof(float), C, layer_id, TA_GPT_CMD_LOAD_LN1B, "LOAD_LN1B"); }
/**
 * @brief 加载注意力block中的QKV矩阵的权重、偏置到TA (形状: 3*C*C)
 */
void load_qkvw_CA(const float *buf, int C, int layer_id) { load_block_tensor_CA(buf, (size_t)(3 * C * C) * sizeof(float), C, layer_id, TA_GPT_CMD_LOAD_QKVW, "LOAD_QKVW"); }
void load_qkvb_CA(const float *buf, int C, int layer_id) { load_block_tensor_CA(buf, (size_t)(3 * C) * sizeof(float), C, layer_id, TA_GPT_CMD_LOAD_QKVB, "LOAD_QKVB"); }
/**
 * @brief 加载注意力block中的输出投影层的权重、偏置到TA (形状: C*C)
 */
void load_attprojw_CA(const float *buf, int C, int layer_id) { load_block_tensor_CA(buf, (size_t)C * C * sizeof(float), C, layer_id, TA_GPT_CMD_LOAD_ATTPROJW, "LOAD_ATTPROJW"); }
void load_attprojb_CA(const float *buf, int C, int layer_id) { load_block_tensor_CA(buf, (size_t)C * sizeof(float), C, layer_id, TA_GPT_CMD_LOAD_ATTPROJB, "LOAD_ATTPROJB"); }
/**
 * @brief 加载前馈网络(FFN)中的第2个LayerNorm的权重、偏置到TA (形状: C)
 */
void load_ln2w_CA(const float *buf, int C, int layer_id) { load_block_tensor_CA(buf, (size_t)C * sizeof(float), C, layer_id, TA_GPT_CMD_LOAD_LN2W, "LOAD_LN2W"); }
void load_ln2b_CA(const float *buf, int C, int layer_id) { load_block_tensor_CA(buf, (size_t)C * sizeof(float), C, layer_id, TA_GPT_CMD_LOAD_LN2B, "LOAD_LN2B"); }
/**
 * @brief 加载前馈网络(FFN)中的第一层(升维)权重、偏置到TA (形状: 4*C*C)
 */
void load_fcw_CA(const float *buf, int C, int layer_id) { load_block_tensor_CA(buf, (size_t)(4 * C * C) * sizeof(float), C, layer_id, TA_GPT_CMD_LOAD_FCW, "LOAD_FCW"); }
void load_fcb_CA(const float *buf, int C, int layer_id) { load_block_tensor_CA(buf, (size_t)(4 * C) * sizeof(float), C, layer_id, TA_GPT_CMD_LOAD_FCB, "LOAD_FCB"); }
/**
 * @brief 加载前馈网络(FFN)中的输出投影层(降维)权重、偏置到TA (形状: 4*C*C)
 */
void load_fcprojw_CA(const float *buf, int C, int layer_id) { load_block_tensor_CA(buf, (size_t)(4 * C * C) * sizeof(float), C, layer_id, TA_GPT_CMD_LOAD_FCPROJW, "LOAD_FCPROJW"); }
void load_fcprojb_CA(const float *buf, int C, int layer_id) { load_block_tensor_CA(buf, (size_t)C * sizeof(float), C, layer_id, TA_GPT_CMD_LOAD_FCPROJB, "LOAD_FCPROJB"); }

// 函数指针，方便将block中每一层的参数传入TEE中
typedef void (*secure_loader_fn)(const float *buf, int C, int layer_id);

/**
 * @brief 模型层数据分发任务台（CA端实现），通过loader回调真正的CA函数，传递相应层的参数给TA（像nemu项目中的实现）
 * @param model_file 已打开的模型文件指针
 * @param params_memory_iterator 指向常规内存中用于存储普通层参数的指针
 * @param L 模型的block总数
 * @param C 模型的特征维度或通道数
 * @param protected_block 需要受保护（隔离存入TA）的block索引
 * @param per_layer 每一层包含的参数数量（即float元素的个数）
 * @param name 张量或层的名称，用于日志调试打印
 * @param loader 安全加载回调函数，负责将受保护的缓冲区数据实际传输至TA
 */
static void load_switch_tensor_CA(FILE *model_file,
                                           float *params_memory_iterator,
                                           int L, int C, int protected_block,
                                           size_t per_layer,
                                           const char *name,
                                           secure_loader_fn loader) {
    size_t pre = (size_t)protected_block * per_layer;
    size_t post = (size_t)(L - protected_block - 1) * per_layer;
    float *secure_buf = (float *)malloc(per_layer * sizeof(float));

    fread(params_memory_iterator, sizeof(float), pre, model_file);
    fread(secure_buf, sizeof(float), per_layer, model_file);
    fread(params_memory_iterator + pre + per_layer, sizeof(float), post, model_file);

    printf("[CA] %s split: layer=%d pre=%zu protected=%zu post=%zu\n",
           name, protected_block, pre, per_layer, post);

    loader(secure_buf, C, protected_block);
    free(secure_buf);
}
 ...
 */
void load_parameters(GPT2 *model, FILE *model_file, int trusted_layer, int protected_block) {
    // 依次加载参数数据
    float* params_memory_iterator = model->params_memory;
    int L = model->config.num_layers;
    int C = model->config.channels;

    if (trusted_layer == 2 && (protected_block < 0 || protected_block >= L)) {
        printf("Error: protected_block %d out of range [0, %d)\n", protected_block, L);
        fclose(model_file);
        exit(1);
    }

    for (size_t i = 0; i < NUM_PARAMETER_TENSORS; i++) {
        if ((trusted_layer == 1 || trusted_layer == 2) && i == 0) {
            // 如果当前参数是wte或wpe，并且可信标志为1，加载入TEE中
            load_parameters_CA(model, model_file);
        ...
        if (trusted_layer == 2 && i == 14) {
            ...
            load_lnfwb_CA(model, model_file);
            printf("Loaded parameters into TEE...\n");
        ...
        if (trusted_layer == 2 && i >= 2 && i <= 13) {
            // 如果当前参数属于“完整注意力块”的保护范围，统一按索引分派
            size_t per_layer = 0;
            const char *tensor_name = NULL;
            secure_loader_fn loader = NULL; // 前面定义的函数指针变量
            ...
            switch (i) {
                ...
                case 4:
                    // qkvw，qkv投影矩阵权重
                    per_layer = (size_t)(3 * C * C);
                    tensor_name = "qkvw";
                    loader = load_qkvw_CA;
                    break;
                case 5:
                    // qkvb，qkv投影矩阵偏置
                    per_layer = (size_t)(3 * C);
                    tensor_name = "qkvb";
                    loader = load_qkvb_CA;
                    break;
                case 6:
                    // attprojw，将多头注意合并回C维的权重
                    per_layer = (size_t)(C * C);
                    tensor_name = "attprojw";
                    loader = load_attprojw_CA;
                    break;
                case 7:
                    // attprojb，将多头注意合并回C维的偏置
                    per_layer = (size_t)C;
                    tensor_name = "attprojb";
                    loader = load_attprojb_CA;
                    break;
                case 8:
                    // ln2w，第二个归一化层权重
                    per_layer = (size_t)C;
                ...
                case 10:
                    // fcw，第一次FFN权重
                    per_layer = (size_t)(4 * C * C);
                    tensor_name = "fcw";
                    loader = load_fcw_CA;
                    break;
                case 11:
                    // fcb，第一次FFN偏置
                    per_layer = (size_t)(4 * C);
                    tensor_name = "fcb";
                    loader = load_fcb_CA;
                    break;
                case 12:
                    ...
                    per_layer = (size_t)(4 * C * C);
                    tensor_name = "fcprojw";
                    loader = load_fcprojw_CA;
                    break;
                case 13:
                    // fcprojb，第二次FFN偏置
                    per_layer = (size_t)C;
                    tensor_name = "fcprojb";
                    loader = load_fcprojb_CA;
                    break;
            }
            // 通过刚刚switch获得的函数指针，进入总控制台函数调用对应的CA实现
            load_switch_tensor_CA(model_file, params_memory_iterator, L, C, protected_block,
                                           per_layer, tensor_name, loader);
 ...
 */
void gpt2_build_from_checkpoint(GPT2 *model, const char* checkpoint_path, int trusted_layer, int protected_block) {
    // 读取检查点文件
    ...
    load_parameters(model, model_file, trusted_layer, protected_block); // 从文件加载参数到内存，考虑trusted_layer的情况
 ...
 * @param T 序列长度
 * @param trusted_layer 可信标志，0-全部在普通环境中运行，1-编码器部分softmax部分在TEE中运行
 * @param protected_block 代表保护第几块block
 * @return 无返回值，但会更新模型激活张量
 */
void gpt2_forward(GPT2 *model, int* inputs, int B, int T, int trusted_layer, int protected_block) {
    ...
    // 如果trusted_layer == 1或2，则编码器部分在TEE中完成
    if (trusted_layer == 1 || trusted_layer == 2) {
    ...
    for (int l = 0; l < L; l++) {
        ...
        float* l_ln1w = params.ln1w + l * C; // params.ln1w代表的是第0层归一化权重的地址
        ...
        if (trusted_layer == 2 && l == protected_block) {
            // 如果是受保护层，则该层完整block在TEE中连续执行
            // 只在最后一步把residual3取回CA，减少中间张量泄露与拷贝开销
            ln1_forward_CA(residual, B, T, C);
            qkv_forward_CA(B, T, C);
            attention_forward_CA(B, T, C, NH);
            attproj_forward_CA(B, T, C);
            residual2_forward_CA(residual, B, T, C);
            ln2_forward_CA(B, T, C);
            fc_forward_CA(B, T, C);
            gelu_forward_CA(B, T, C);
            fcproj_forward_CA(B, T, C);
            residual3_forward_CA(B, T, C);
            block_output_CA(l_residual3, B, T, C);
        } else {
    ...
    // 如果trusted_layer == 2，则最终的matmul和softmax在TEE中完成
    if (trusted_layer == 2) {
        // 通过TEE接口完成最后的计算      
        // matmul_forward(acts.logits, acts.lnf, params.wte, NULL, B, T, C, V);
        // softmax_forward(acts.probs, acts.logits, B, T, V);
...
int main(int argc, char *argv[]) {
    ...
    int protected_block = 0; // 默认保护第0个block
    
    // 解析命令行参数
    if (argc < 3) {
        printf("Usage: %s <model_path> <tokenizer_path> [-T <trusted_layer_flag>] [-P <layer_id>]\n", argv[0]);
        return 1;
    }
    model_path = argv[1];
    ...
    for (int i = 3; i < argc; i++) {
        if (strcmp(argv[i], "-T") == 0) {
            if (i + 1 >= argc) {
                printf("Error: -T requires a value\n");
                return 1;
            }
            trusted_layer_flag = atoi(argv[i + 1]);
            i++; // 跳过数值
    ...
    // 加载GPT2模型
    GPT2 model;
    gpt2_build_from_checkpoint(&model, model_path, trusted_layer_flag, protected_block);
    ...
    while (current_len < max_total && current_len - num_input_tokens < max_gen) {
        // 以当前token序列前向传播
        gpt2_forward(&model, tokens, 1, current_len, trusted_layer_flag, protected_block);
